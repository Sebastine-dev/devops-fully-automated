...
pipeline {
    ...
    stages {
        stage('Git checkout') {
            steps {
                ...
                git branch: 'main', url: 'https://github.com/Sebastine-dev/devops-fully-automated.git'
        ...
        stage('Deploy to DEV env') {
            ...
            steps {
                withCredentials([usernamePassword(credentialsId: 'ansible-deploy-server-credentials', passwordVariable: 'PASSWORD', usernameVariable: 'USER_NAME')]) {
                    sh "ansible-playbook -i ${WORKSPACE}/ansible-setup/aws_ec2.yaml ${WORKSPACE}/deploy.yaml --extra-vars "ansible_user=$USER_NAME ansible_password=$PASSWORD hosts=dev_server workspace_path=$WORKSPACE""
                }
            }
        }
    ...
    post {
        always {
            ...
            slackSend channel: 'cyberhome', color: COLOR_MAP[currentBuild.currentResult], message: "*${currentBuild.currentResult}:* Job ${env.JOB_NAME} build ${env.BUILD_NUMBER} \n More info at: ${env.BUILD_URL}"
        }
    }
}
